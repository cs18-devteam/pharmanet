...
class Model {
  ...
  #buildCreateTableQuery() {
    try {
      ...
      let query =
        "CREATE TABLE IF NOT EXISTS %%TABLE_NAME%%(%%COLUMN_DATA%%  %%TABLE_CONSTRAINTS%% )";
      ...
      for (const [name, value] of Object.entries(this)) {
        ...
        columnQueryTemplate = columnQueryTemplate.replace(
          "%%COLUMN_NAME%%",
          name,
        );
        ...
        if (value.foreignKey) {
          ...
          tableConstraints.push(
            `FOREIGN KEY (${name}) REFERENCES ${tableName}(${foreignKey})`,
          );
        }
  ...
  async save(data) {
    try {
      ...
      const createdUser = await db.query(
        `select * from ${this.#table} where id=${results.insertId}`,
      );
...
>>>>>>> kasum/admin/3.7
  ...
  async query(query) {
    try {
      ...
      const results = await db.query(
        query.replace("this.table", this.#table),
        queryHandler,
      );
      return results;
    } catch (e) {
