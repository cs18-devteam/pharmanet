...
exports.sendOnlinePharmacies = apiCatchAsync(async (req, res) => {
    ...
    if (cartList) {
        ...
        medicineIds = await Promise.all(carts.map(async id => {
            const [cart] = await Carts.getById(id);
            if (!cart) return undefined;
            return cart.medicineId;
        }))

        medicineIds = medicineIds.filter(m => m != undefined);

    }

    for (const [id, client] of Object.entries(connectedPharmacies)) {
        pharmacies.push((await Pharmacies.getById(id))[0]);
    }
    ...
    if (medicineIds.length) {
        ...
        await Promise.all(medicineIds.map(async m => {
            ...
            pharmacies = pharmacies.filter(p => p != undefined);
        }));
    }
    ...
    if (latitude && longitude) {
        pharmacies = pharmacies.map(p => {
            p.distance = calculateDistanceKM(latitude, longitude, p.latitude, p.longitude);
            p.distance = Number(p.distance).toFixed(2);
            return p;
        })
