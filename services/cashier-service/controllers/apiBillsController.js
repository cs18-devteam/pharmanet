...
exports.getBills =async (req , res)=>{
    ...
    if(Object.entries(filter).length > 0){
        content = await Bills.get(filter);
    }else{
        content = await Bills.get();
    }
...
exports.deleteBills =async (req , res)=>{
    ...
    const results = await Bills.deleteById(id);
...
exports.updateBills = async (req , res)=>{
    ...
    const results = await Bills.update({
...
exports.createBills = async (req , res)=>{
    const {user , requestedDate , acceptedDate , dateFrom , dateTo , reason , acceptedBy} = JSON.parse(await getRequestData(req));
    const newBillsRequest = await Bills.save({
        user , requestedDate , acceptedDate , dateFrom , dateTo , reason , acceptedBy
    });
    
    return responseJson(res , 201 , {
        status: 'success',
        data: newBillsRequest,
        count : newBillsRequest.length,
    });
}